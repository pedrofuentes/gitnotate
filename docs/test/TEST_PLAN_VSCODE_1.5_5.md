...
| 38.12 | Inline diff view behavior | Set `diffEditor.renderSideBySide` to `false`. Open a markdown diff. | Observe comment behavior. Document any issues — this may need follow-up handling. | ⚠️ Known limitation: inline diff shows right-side comments+highlights only. Left-side comments don't appear. Single file view shows no highlights. |
