...
| # | Test | Steps | Expected | Status |
|---|------|-------|----------|--------|
| 18.1 | Save markdown triggers re-sync | Open `edge-cases.md` (has `^gn` comments). Edit the file (add a blank line). Save (`Ctrl+S`). | Debug Console shows `[Gitnotate] Document saved: edge-cases.md` followed by a comment sync. Threads remain correct after the save. | ✅ Unit + 🔍✅ Integration + Manual verified |
| 18.2 | Save non-markdown does NOT trigger sync | Open `sample.js`. Edit it. Save (`Ctrl+S`). Check Debug Console. | NO `[Gitnotate] Document saved:` log entry. No comment sync triggered. | ✅ Unit + 🔍✅ Integration + Manual verified |
| 18.3 | Save refreshes after line shift | Open `edge-cases.md` (has `^gn:7:R:23:112` comment on line 7). Add 2 blank lines above line 7. Save. | After save, the thread stays on line 7 per metadata (even though the text shifted to line 9) — this is expected because `^gn` metadata is static. The re-sync confirms no crash occurs. | 🔍✅ Integration + Manual verified |
