...
| 9.4 | Non-`^gn` comments shown as line threads | Have regular line-level PR comments (without `^gn` metadata) on the same file. | Regular comments appear as full-line threads (no sub-line range, no wavy underline). Comment body shows the full text as-is. | ⬜ |
